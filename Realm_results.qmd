...
Reformat the data to ensure that the results can be "bound" in to a single table for each metric RLE count, RLE extent, EPL count, EPL extent. Adding realm column to ensure than in bound data one can id the rows for each realm.
...
Repeat data compilaiton for EPL
...
epl_count_all <- bind_rows(ter_epl_count, mar_epl_count, est_epl_count, riv_epl_count, wet_epl_count, coa_epl_count, sub_epl_count) %>%
...
epl_ext_all <- bind_rows(ter_epl_ext, mar_epl_ext, est_epl_ext, riv_epl_ext, wet_epl_ext, coa_epl_ext, sub_epl_ext) %>%
  rename( `Well Protected` = WP, 
         `Moderately Protected` = MP, 
         `Poorly Protected` = PP, 
         `Not Protected` = NP ) %>%
